...
class StockHistoryAPIQuerySet(models.QuerySet):
    ...
    def filter(self, **kwargs):
        ...
        if 'stock_symbol__symbol' in kwargs:
            ...
            logger.info(f"[FILTER] Set symbol from stock_symbol__symbol: {clone._symbol}")
            ...
            logger.info(f"[FILTER] Set symbol from stock_symbol: {clone._symbol}")
    ...
    def _get_api_data(self) -> List[Dict]:
        ...
        if not symbol:
            if 'stock_symbol' in self._filters_dict:
                ...
                logger.info(f"[GET_API_DATA] Extracted symbol from filters: {symbol}")
        
        if not symbol:
            logger.warning(f"[GET_API_DATA] No symbol set. Filters: {list(self._filters_dict.keys())}")
            return []
        ...
        try:
            ...
            # Fetch from API
            logger.info(f"[GET_API_DATA] Calling API: symbol={symbol}, start_date={start_date}, end_date={end_date}, is_hourly={is_hourly}")
            ...
            logger.info(f"[GET_API_DATA] API call returned")
            data = None
            if response:
                logger.info(f"[GET_API_DATA] Response type: {type(response).__name__}")
                if isinstance(response, dict):
                    logger.info(f"[GET_API_DATA] Response keys: {list(response.keys())}")
                    # Check multiple possible key names
                    for key in ['history', 'data', 'results', 'records', 'stock_history']:
                        if key in response:
                            logger.info(f"[GET_API_DATA] Found key '{key}' in response")
                            data = response[key]
                            logger.info(f"[GET_API_DATA] Data type for '{key}': {type(data).__name__}")
                            if isinstance(data, list):
                                logger.info(f"[GET_API_DATA] Retrieved {len(data)} records from '{key}'")
                                transformed_data = []
                                for idx, item in enumerate(data):
                                    try:
                                        transformed = {
                                            ...
                                            'is_hourly': item.get('is_hourly', False)
                                        }
                                        if idx < 1:
                                            logger.debug(f"[GET_API_DATA] Sample item keys: {list(item.keys())}")
                                            logger.debug(f"[GET_API_DATA] Sample transformed keys: {list(transformed.keys())}")
                                        transformed_data.append(transformed)
                                    except Exception as e:
                                        logger.error(f"[GET_API_DATA] Error transforming item {idx}: {str(e)}")
                                        continue
                                logger.info(f"[GET_API_DATA] Successfully transformed {len(transformed_data)} records")
                                data = transformed_data
                                break
                else:
                    logger.warning(f"[GET_API_DATA] Response is not a dict, type: {type(response).__name__}")
                    return []
            else:
                logger.warning(f"[GET_API_DATA] Response is None or falsy")
                return []
            
            if not data:
                logger.warning(f"[GET_API_DATA] No data found in response")
                return []
    ...
    def __iter__(self):
        ...
        try:
            ...
            yielded_count = 0
            logger.info(f"[ITER] Beginning to yield items")
            for idx, item in enumerate(api_data):
                try:
                    if idx < 1 or idx % 1000 == 0:
                        logger.debug(f"[ITER] Yielding item {idx}, keys: {list(item.keys()) if isinstance(item, dict) else 'not a dict'}")
                    # Return raw dict without model instantiation - much faster!
                    yield item
                    yielded_count += 1
                except Exception as e:
                    logger.error(f"[ITER] Error yielding item {idx}: {str(e)}", exc_info=True)
                    continue
            
            logger.info(f"[ITER] Finished: yielded {yielded_count} items total")
        except Exception as e:
    ...
    def count(self):
        """Count results."""
        try:
            logger.info(f"[COUNT] Calling _get_api_data")
            api_data = self._get_api_data()
            count = len(api_data)
