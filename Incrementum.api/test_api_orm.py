...
def test_database_models():
    ...
    try:
        ...
        if DBStockModel.objects.exists():
            ...
            # Test history
            history_count = DBStockHistory.objects.filter(stock_symbol=first_stock).count()
...
def test_api_models():
    ...
    try:
        ...
        if len(stocks) > 0:
            ...
            history = APIStockHistory.objects.filter(stock_symbol__symbol=first_stock.symbol)
...
def test_conditional_imports():
    ...
    print(f"With API disabled - StockModel class: {StockModel.__module__}.{StockModel.__name__}")
    print(f"With API disabled - StockHistory class: {StockHistory.__module__}.{StockHistory.__name__}")
