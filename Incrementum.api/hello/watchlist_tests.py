...
class WatchlistServiceTest(TestCase):
    ...
    def test_add_and_get_stock(self):
        watchlist = self.service.add(self.account.id, "AAPL")
        assert any(stock['symbol'] == "AAPL" for stock in watchlist)

        watchlist = self.service.add(self.account.id, "TSLA")
        assert any(stock['symbol'] == "TSLA" for stock in watchlist)
        assert len(watchlist) == 2

    def test_remove_stock(self):
        self.service.add(self.account.id, "AAPL")
        self.service.add(self.account.id, "TSLA")
        watchlist = self.service.remove(self.account.id, "AAPL")
    ...
    def test_search_stock(self):
        self.service.add(self.account.id, "AAPL")
        self.service.add(self.account.id, "TSLA")
        results = self.service.search(self.account.id, "AAP")
    ...
    def test_watchlist_is_user_specific(self):
        ...
        self.service.add(self.account.id, "AAPL")
        self.service.add(account2.id, "TSLA")
        wl1 = self.service.get(self.account.id)
        wl2 = self.service.get(account2.id)
